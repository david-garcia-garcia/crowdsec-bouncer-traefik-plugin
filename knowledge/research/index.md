# Research

## traefik
priority: normal
local: index_ext_traefik.md
description: Official Traefik plugin-load behavior this product relies on.

## crowdsec
priority: normal
local: index_ext_crowdsec.md
description: Official Crowdsec Docker and cscli behavior this product relies on.

## simpleredis
priority: normal
local: index_ext_simpleredis.md
description: SimpleRedis client API and pooling behaviour this product relies on.

## dragonfly
priority: normal
local: index_ext_dragonfly.md
description: Official Dragonfly Redis-protocol and Docker image behaviour this product relies on.

## redis
priority: normal
local: index_ext_redis.md
description: Official Redis Go client surface and toolchain constraints.
