...
_Avoid_: requiring them on hCaptcha, classic `recaptcha`, Turnstile, `custom`, or `eucaptcha`
...
- Do not require those knobs on any other provider. Leftover empty enterprise fields on classic `recaptcha` and `eucaptcha` are ignored.
