...
**Forced decision header**:
The Config string `bouncerDecisionHeader` naming an incoming request header whose exact trimmed values `b` (ban) and `c` (captcha) force that remediation. Ban skips stream/live lookup. Captcha still looks up so an existing ban wins. Empty means the feature is off. Not `lapiScopeHeaders` (CrowdSec identity). Not `bouncerRemediationHeader` (outgoing).
