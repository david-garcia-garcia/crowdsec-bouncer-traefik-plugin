...
- Then loop `stream.New`: Ip/header `PutMany` in `PutManyChunk` flushes, Range CIDRs into upserts via `KindOriginString`. Map kind with `RemediationValue` after `MetricsOrigin`; `OriginBasedDecisionRemap` is Bouncer apply (`core_plugin_lapi_origin-based-decision-remap.md`), not a store rewrite.
