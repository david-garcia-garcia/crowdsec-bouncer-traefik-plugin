...
- Then loop `stream.New`: Ip/header `PutMany` in `PutManyChunk` flushes, Range CIDRs into upserts via `KindOriginString`, `rememberActiveDecision`. Map kind with `remediationKindForOrigin` after `MetricsOrigin` (`core_plugin_lapi_ban-to-captcha-origins.md`); do not call `RemediationValue` alone on those writes.
