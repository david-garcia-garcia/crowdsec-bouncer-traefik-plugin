...
- `allow_decisions.Tests.ps1` asserts CrowdSec `type=allow` as a whitelist over a ban. Allow-only already passes; allow-over-ban is currently red (`strongestLiveDecision` returns the first ban and stream skips unknown types).
- LAPI/AppSec failure-action routes point at `crowdsec:9` with `httpTimeoutSeconds=2`. Do not `docker pause` Crowdsec; that would stall the shared LAPI. Readiness only waits for `/lapi-fail-ban` to answer; `failure_action.Tests.ps1` is the fail-closed assertion. If that case returns 200, Traefik logs `lapi session joiner adopted` for `bouncer-lapi-fail-ban@docker` on the healthy `crowdsec:8080` none-mode session.
