...
- Traefik e2e logs are `--log.format=json` and `--log.nocolor=true`. Plugin labels and file-provider YAML set `logFormat: json`.
...
- Nested plugin maps (`decisionScopeHeaders`, geoblock `databaseSources`) MUST use the file provider (`tests/e2e/real/config/dynamic-scopes.yml`). Docker labels do not decode those maps.
...
- `tests/e2e/real/config/dynamic-scopes.yml`
