...
- Custom-ban and captcha compose labels set `bouncerBanFilePath` / `captchaFilePath`. Captcha-serving routes also set `captchaEnabled: true` (empty name fills). Do not use `banHtmlFilePath` / `captchaHtmlFilePath` or leftover `bouncerCaptcha*`.
