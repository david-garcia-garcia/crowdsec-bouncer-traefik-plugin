...
- Usage-metrics cases (`usage_metrics.Tests.ps1`) need `metricsUpdateIntervalSeconds=1` on `/stream` and `/appsec`. Default 600s would miss CI. Assert with `cscli metrics show bouncers -o json` (origin `cscli` / `appsec`; processed is empty origin). `/trusted` may keep the default 600s: stream/alone share one ticker per LAPI URL+key (warn-and-wire). Do not copy `/stream`’s metrics label onto `/trusted` as a uniqueness workaround. The file-provider stream bouncer still uses a second fixture key (`BOUNCER_KEY_TRAEFIK_SCOPES`) so `scopes=` isolation is a different CrowdSec bouncer row.
