...
- Usage-metrics cases (`usage_metrics.Tests.ps1`) need `metricsUpdateIntervalSeconds=1` on `/stream` and `/appsec`. Default 600s would miss CI. Assert with `cscli metrics show bouncers -o json` (origin `cscli` / `appsec`; processed is empty origin).
