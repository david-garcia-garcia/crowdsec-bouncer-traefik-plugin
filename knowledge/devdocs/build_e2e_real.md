...
- Usage-metrics cases (`usage_metrics.Tests.ps1`) need `metricsUpdateIntervalSeconds=1` on `/stream` and `/appsec`. Default 600s would miss CI. Assert with `cscli metrics show bouncers -o json` (origin `cscli` / `appsec`; processed is empty origin). That interval is part of CrowdsecConnection reclaim identity. Every other stream middleware on `BOUNCER_KEY_TRAEFIK_TEST` (`/trusted`) MUST use the same value or two pollers share one LAPI stream session and `/stream` never sees bans.
