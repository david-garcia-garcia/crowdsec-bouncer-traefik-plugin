...
- `newTransport` sets `http.Client.Timeout` and stored `httpTimeoutSeconds` from `cfg.EffectiveHTTPTimeoutSeconds(cfg.LapiHttpTimeoutSeconds)`. Do not read raw `HTTPTimeoutSeconds` when the LAPI override is non-zero. Store effective seconds so `fieldsDiffer` sees a shared-default change when the override is still 0.
- Write the CAPI token on the stored transport (`getToken`). Do not keep a write-once Client key field beside it.
- Pass `bouncerLiveTtlSeconds` into `LiveLookup`. Do not store that TTL on Client.
...
- Keep `LapiStreamStartupBlock` write-once at `startStream`. First incarnation keeps it. Do not put it on Bouncer. Do not mutate it after construct.
...
value, err := client.LiveLookup(remoteIP, scopes, bouncerLiveTtlSeconds)
```
...
- The IP-key TTL follows the IP query: an active `?ip=` result uses `liveCacheTTL` on that result's duration; a clean `?ip=` result uses `bouncerLiveTtlSeconds`. Do not apply a header winner's duration to the IP key.
...
- `sessionKey` is write-once in `New` (`reclaimSessionKey`). Do not assign it after `OpenWithHooks` returns: Wake already started `go handleStreamTicker()`, which reads `sessionKey` for poll/skip logs.
- Redis host/auth/db stay on the Client Open key (`core_plugin_lapi_reclaim-key.md`). Stream intervals, CAPI scenarios, `lapiUpdateMaxFailure`, and `lapiScopeHeaders` are not on that key (silent first-wins). Live/none `Key` keeps `LapiMetricsIntervalSeconds`. Do not call `PeekLivePrefix`.
