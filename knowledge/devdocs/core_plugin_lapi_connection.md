...
- Pass the inbound request Context and `defaultDecisionSeconds` into `LiveLookup`. Do not use constructor ctx or `context.Background()`. Do not store that TTL on Client. Live GET admission is `core_plugin_lapi_backend-backoff.md`.
...
value, err := client.LiveLookup(req.Context(), remoteIP, scopes, defaultDecisionSeconds)
```
...
- Redis host/auth/db stay on the Client Open key (`core_plugin_lapi_reclaim-key.md`). Stream intervals, CAPI scenarios, `updateMaxFailure`, backend backoff knobs, and `decisionScopeHeaders` are not on that key (silent first-wins). Live/none `Key` keeps `MetricsUpdateIntervalSeconds`. Do not call `PeekLivePrefix`.
