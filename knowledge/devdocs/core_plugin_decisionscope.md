...
- Resolve the client IP with `pkg/ip.GetRemoteIP`. Then `LookupCachedRemediation` with `conn.RangeMembership()` in stream/alone/live cache hits (kind, origin, err). Pass `client.parsed` into Range membership. Matching uses the first letter; origin is for usage-metrics only. Do not put scopes on `clientRequest`.
...
kind, origin, err := decisionscope.LookupCachedRemediation(cacheClient, mode, client.remoteIP, client.parsed, scopes, conn.RangeMembership())
conn.IncDropped(origin, client.ipType, "ban")
```
