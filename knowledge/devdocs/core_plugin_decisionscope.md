...
- Resolve the client IP with `pkg/ip.GetRemoteIP`. Then `LookupCachedRemediation` with `conn.RangeMembership()` in stream/alone/live cache hits (kind, origin, err). Pass the parsed `net.IP` into Range membership. Matching uses the first letter; origin is for usage-metrics only.
...
kind, origin, err := decisionscope.LookupCachedRemediation(cacheClient, mode, remoteIP, parsedIP, scopes, conn.RangeMembership())
conn.IncDropped(origin, ip.FamilyOfIP(parsedIP), "ban")
```
