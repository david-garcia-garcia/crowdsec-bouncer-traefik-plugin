...
**AppSec Client**:
The reclaim value for one CrowdSec AppSec listener: HTTP client, host, key, TLS, body limit. Keyed by AppSec URL+key+TLS. Not the LAPI Connection.
...
**Failure action**:
The operator enum (`passthrough` | `ban` | `captcha`) this plugin applies when LAPI or AppSec does not return a usable verdict. LAPI action is on LAPI Connection identity; AppSec action is per-router on Bouncer. Default is `ban`.
...
- `crowdsecLapiFailureAction` is on LAPI Connection identity (shared with `updateMaxFailure`). `crowdsecAppsecFailureAction` stays on Bouncer.
