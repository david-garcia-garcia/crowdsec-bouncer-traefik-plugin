...
- Put stream tickers, replaceable LAPI HTTP (`transport` on `atomic.Value`), cache, and Range membership on `lapi.Client`. Put AppSec HTTP on `appsec.Client`. Put captcha, templates, LAPI failure action, Redis fail-closed, and live-cache TTL on Bouncer. After `OpenStream` / `OpenLive`, `AdoptTransport` last-wins TLS/timeout.
...
- Do not put middleware name, `next`, ban/captcha templates, trusted IPs, Enabled, AppSec knobs, LAPI failure action, Redis fail-closed, live-cache TTL, `StreamStartupBlock`, HTTP timeout, or LAPI TLS in the LAPI reclaim key.
...
- Lifecycle INFO lines: `crowdsec connection started|sleeping|waking|closed`. Stream health transitions: `crowdsec stream became unhealthy|healthy` (not every poll). Cache tick messages `handleStreamCache:updated` and `handleStreamCache:alreadyUpdated` are DEBUG.
