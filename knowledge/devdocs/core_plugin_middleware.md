...
- After trusted-IP skip and forced `b`, `bouncerLapiExcludeRegex` / `bouncerAppsecExcludeRegex` skip that CrowdSec leg when the compiled RE2 matches `{host}/path`. Host is `req.Host` (port stripped with `net.SplitHostPort` when that succeeds). Path is `req.URL.Path` (empty → `/`). Empty after trim is off. Unanchored `MatchString`. Do not hash these strings into LAPI ownership or AppSec identity. Do not call `captcha.RequestDomain` or rebuild Host.
