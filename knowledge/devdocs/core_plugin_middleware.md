...
- Put stream tickers, LAPI HTTP, cache, and Range membership on `lapi.Client`. Put AppSec HTTP on `appsec.Client`. Put captcha and templates on Bouncer. Custom captcha JS/challenge pass-through lives on the Captcha Client (`core_plugin_captcha.md`).
