...
**LAPI Client**:
The reclaim value for one CrowdSec LAPI/CAPI decisions backend: stream/metrics tickers, LAPI HTTP, a child DecisionStore, and in-process Range membership. Stream/alone: keyed by cursor SessionHex (`lapi:stream:`). Live/none: keyed by SessionHex plus a hash of Redis store params and `MetricsUpdateIntervalSeconds` (`lapi:`). Not middleware name. `IdentityHex` is not the live Open suffix.
...
**Stream session**:
The CrowdSec bouncer row this process polls: LAPI scheme, host, and path plus lapiKey (CAPI machine and password in alone). Settings such as Redis and metrics interval are not the session. A live joiner `Open`s the same SessionKey.
_Avoid_: middleware name, outbound IP, IdentityHex as the Open suffix, `scopes=`, AppSec host, PeekLivePrefix
...
- Stream/alone: CrowdSec stores one `GET /v1/decisions/stream` cursor per hashed API key plus the IP LAPI sees (this process’s outbound address). Reclaim `Open` key is `lapi:stream:` plus SessionHex. A second stream `New` on that session `Open`s that key. Redis, interval, CAPI scenario, and `updateMaxFailure` mismatch is first-wins with WARN. Do not call `Peek` / `PeekLivePrefix`. Last New `AdoptTransport`s TLS/timeout (INFO `adopted`). Last holder Sleeps tickers; reload Wakes (`startup=false`) even when Redis YAML changed. Isolated backends need a second bouncer key. Live/none `Key` is `lapi:` plus SessionHex plus Redis and `MetricsUpdateIntervalSeconds` (AppSec excluded). `IdentityHex` is not the live Open suffix.
- LAPI `Close()` stops tickers, idle LAPI HTTP, and the child DecisionStore. Sleep and Wake keep the store. AppSec `Close()` releases idle AppSec HTTP. Do not use `sync.Once`.
