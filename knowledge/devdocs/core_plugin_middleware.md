...
- `CrowdsecConnection.ReclaimGrace` is 30s, set on the slot when the connection is put. `reclaim.DefaultGrace` stays 10s. `ResetForTestWith` does not shorten a CrowdsecConnection slot.
