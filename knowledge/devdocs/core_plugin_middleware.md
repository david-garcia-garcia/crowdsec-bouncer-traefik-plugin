...
- Call `lapi.Prepare` then `appsec.Prepare` (AppSec key/scheme copy only when AppSec is enabled). `PrepopulateInstanceNames` only when that leg is owned. Stream/alone: `lapi.OpenStream`. Live/none: `lapi.OpenLive`. When `crowdsecAppsecEnabled`: `appsec.Open`. Then `instance.PublishAll`. `bouncer.New` takes subscribe flags, not client pointers. Subscribe after New. Open key: `core_plugin_lapi_reclaim-key.md`. Stream `scopes=`: `core_plugin_lapi_scope-union.md`.
