...
- Pass `req.Request.Context()` and `config.DefaultDecisionSeconds` into `LiveLookup`. Do not use constructor ctx or `context.Background()`. Two routers on one Client last-write that TTL into the shared live cache. Live GET admission is `core_plugin_lapi_backend-backoff.md`.
...
- Do not put middleware name, `next`, ban/captcha templates, trusted IPs, Enabled, AppSec knobs, LAPI failure action, Redis fail-closed, live-cache TTL, `StreamStartupBlock`, HTTP timeout, LAPI TLS, or backend backoff knobs in the LAPI reclaim key. Do not put AppSec TLS, HTTP timeout, or backend backoff knobs in the AppSec reclaim key.
