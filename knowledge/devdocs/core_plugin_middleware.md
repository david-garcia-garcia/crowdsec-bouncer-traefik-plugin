...
- Call `lapi.Prepare` then `appsec.Prepare`. Stream/alone: `lapi.OpenStream` (Peeks the DecisionStore key, then registers this bind ctx on the live-router scope union). Live/none: `lapi.OpenLive`. A different Traefik `name` on the same SessionHex fails `New` before Open. `crowdsecMode: appsec`: skip LAPI Open. When `crowdsecAppsecEnabled`: `appsec.Open` (`AdoptTransport` inside). Return `bouncer.New(..., lapiClient, appsecClient, ...)`. Open key: `core_plugin_lapi_reclaim-key.md`. Stream `scopes=`: `core_plugin_lapi_scope-union.md`.
