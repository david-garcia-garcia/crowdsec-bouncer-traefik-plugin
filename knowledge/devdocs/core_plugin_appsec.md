...
- AppSec HTTP 500 and unreachable (transport failure or listener HTTP 502/503/504) use per-router `crowdsecAppsecFailureAction` (`passthrough` | `ban` | `captcha`), not the three removed block bools. An HTTP/2+ body that cannot be buffered is a headers-only GET to AppSec, not a failure-action drop. `captcha` here is `pkg/captcha`, not AppSec JSON `action: captcha`.
