...
- Admit each `Do` through the Gate (`core_plugin_appsec_backend-backoff.md`) before `httpClient.Do`. Inbound unreadable body that never `Do`s: no Allow.
...
- Do not put AppSec TLS, `HTTPTimeoutSeconds`, or backend backoff knobs in the AppSec reclaim key. Last `New` `AdoptTransport`s TLS/timeout. Concurrent adopt last-writes and idle-closes the replaced `*http.Client`.
