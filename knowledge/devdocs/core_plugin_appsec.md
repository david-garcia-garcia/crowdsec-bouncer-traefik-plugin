...
- AppSec HTTP 500, unreachable, unreadable body, and response body read failures use per-router `crowdsecAppsecFailureAction` (`passthrough` | `ban` | `captcha`), not the three removed block bools. `captcha` here is `pkg/captcha`, not AppSec JSON `action: captcha`.
- `crowdsecAppsecBodyLimit: 0` means unlimited forward to AppSec (full POST body). Omitted key defaults to 10 MiB truncation.
...
- Forward path strips hop-by-hop and stale body-size headers; `Content-Length` on the AppSec request matches bytes sent after truncation.
