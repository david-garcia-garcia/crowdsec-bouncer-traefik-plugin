...
**AppSec Client**:
The reclaim value for one CrowdSec AppSec listener (`pkg/appsec`). Keyed by middleware name plus host, path, resolved key, body limit, TLS, and `appsecHttpTimeoutSeconds`. Owns JSON parse and HTTP+auth. Not the LAPI Client.
...
**Client disconnected**:
The client stopped sending a readable AppSec-forwardable body mid-copy (`ErrClientDisconnected`: `context.Canceled`, `context.DeadlineExceeded`, or `io.ErrUnexpectedEOF`). Not an AppSec verdict and not `bouncerAppsecFailureAction`. The bouncer stops without origin, logs TRACE, and may set `bouncerRemediationHeadersCustomName` to `error:client-disconnected`.
...
- AppSec TLS, body limit, and `appsecHttpTimeoutSeconds` live on the ownership key. A change is a new Client. Do not apply those knobs with `AdoptTransport`.
