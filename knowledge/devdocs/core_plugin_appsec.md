...
**Client body dropped**:
A classified client disconnect or cancel while `io.ReadAll` buffers a readable forwardable body in `newAppsecBodyRequest` (`errClientBodyDropped`; operator-visible `appsecQuery:clientBodyDropped`). `Query` maps it through `crowdsecAppsecFailureAction`, not the unclassified `GetBody` ban path.
_Avoid_: GetBody error, unreadable body, HTTP/2 CANCEL (without FailureAction)
...
- AppSec HTTP 500, unreachable (transport failure or listener HTTP 502/503/504), AppSec response-body io errors, an unreadable HTTP/2 or HTTP/3 body on POST, PUT, or PATCH, and a **client body dropped** while buffering a readable forwardable body (POST, PUT, PATCH, DELETE) use per-router `crowdsecAppsecFailureAction` (`passthrough` | `ban` | `captcha`), not the three removed block bools. `captcha` here is `pkg/captcha`, not AppSec JSON `action: captcha`. A response-body io error keeps `appsecQuery:readBody`. Client-gone during buffer uses `appsecQuery:clientBodyDropped`. Unclassified client-body read faults keep `appsecQuery:GetBody` and bypass FailureAction. Oversized AppSec bodies do not use this action.
...
- Three client-body FailureAction shapes. **Unreadable** (`isBodyUnreadable`: HTTP/2+ without Content-Length): passthrough still queries AppSec with headers-only GET; ban on POST/PUT/PATCH drops without AppSec. **Client body dropped** (`isClientGoneBodyReadErr` during buffer `io.ReadAll`): `Query` uses `resultForFailureAction` with `appsecQuery:clientBodyDropped`; passthrough allows without calling AppSec (not headers-only GET). **Unclassified** `GetBody` read errors: keep `fmt.Errorf("appsecQuery:GetBody %w", err)` and today's ban wiring; FailureAction does not apply.
- Client-gone is `errors.Is` for `context.Canceled`, `context.DeadlineExceeded`, and `io.ErrUnexpectedEOF` in `isClientGoneBodyReadErr` only. Mid-read cancel on a readable CL-known body is client body dropped, not unreadable; do not extend `isBodyUnreadable` for that case.
