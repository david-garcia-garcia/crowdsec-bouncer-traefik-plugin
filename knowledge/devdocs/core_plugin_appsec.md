...
- AppSec HTTP 500 and unreachable (transport failure or listener HTTP 502/503/504) use per-router `crowdsecAppsecFailureAction` (`passthrough` | `ban` | `captcha`), not the removed `crowdsecAppsecFailureBlock` / `crowdsecAppsecUnreachableBlock`. An HTTP/2+ body that cannot be buffered is a headers-only GET to AppSec unless per-router `crowdsecAppsecUnreadableBodyBlock` is true (then POST/PUT/PATCH/DELETE drop before AppSec; GET/HEAD still headers-only). `captcha` here is `pkg/captcha`, not AppSec JSON `action: captcha`.
