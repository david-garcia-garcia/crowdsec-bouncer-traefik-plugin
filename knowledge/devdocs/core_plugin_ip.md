...
**clientRequest**:
The inbound request plus the client address GetRemoteIP already chose (`remoteIP` string, parsed `net.IP`, `ipType` for metrics). Handlers keep the parameter name `req`.
...
- Resolve the client address with `GetRemoteIP` (server/trusted-hop pool + custom header). Put that string, the parsed `net.IP`, and `FamilyOfIP` on `clientRequest`. Keep the name `req`. Then `ContainsIP` on `req.parsed` for the client pool. Do not parse `RemoteAddr` again. Do not parse the chosen string again for trusted-client membership. Do not add scopes or origin to `clientRequest`.
...
ok := checker.ContainsIP(req.parsed)
```
