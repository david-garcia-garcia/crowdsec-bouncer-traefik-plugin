...
- Unchanged reload Sleeps then Wakes (grace does not Close). Changed snapshot with no live holder: ReplaceSleeping then create.
...
1. **Session key** = mode (`stream`/`alone`) + LAPI scheme/host/path + lapiKey, or CAPI machine+password in alone. Not middleware name. TLS extras, Redis, intervals, AppSec, `decisionScopeHeaders` are the **settings snapshot**, not the session. Alternative: put scopes in the hash — that starts a second poller on the same CrowdSec cursor.

2. **Warn-and-wire** when Peek shows live holders and the snapshot differs. Log owner middleware (`streamOwner` on the connection), joiner, and field names. Share the existing `CrowdsecConnection`. Alternative: fail `New` — louder, takes the joiner router down.

3. **Sleep / Wake / grace Close.** Last holder → `Sleep()` (tickers off, object kept). `Open` during grace → `Wake()` (`startup=false`). Grace elapsed → table `Close()`s. Snapshot change on a sleeper → `ReplaceSleeping` (internal dispose, then create). Callers do not Close slots. Unchanged snapshot: no extra `startup=true`. Alternative: keep tickers running during grace — reload gap would still poll, but a racing new snapshot could overlap.

4. **`Peek` / `ReplaceSleeping` on `pkg/reclaim`** (stdlib table). Peek is ownership/snapshot without bind. Alternative: `sync.Once` — never disposes. Alternative: public `DropNow` — exposes slot Close.
...
- [ReplaceSleeping/Open race] → live holders skip discard; Open’s create-race still drops the extra constructor.
