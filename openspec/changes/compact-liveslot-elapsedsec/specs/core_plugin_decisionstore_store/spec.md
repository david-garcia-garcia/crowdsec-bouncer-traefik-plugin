...
The decisionstore package SHALL fix a process-wide origin once at package init as wall Unix minus two seconds. Elapsed `now()` SHALL be wall Unix minus that origin, clamped so it never decreases when wall clock steps backward. Packing a memory slot from CrowdSec `durationSec` SHALL set `expiresAt` to a saturated int32 in the open interval `(1, MaxInt32]` derived from elapsed `now()` plus `durationSec`, so duration `0` and `-1` yield values that miss under the predicate `expiresAt > 0 && expiresAt <= now()` on the next comparison. The Store `PublishTick` method and the memory engine PublishTick callback SHALL take `now` as int32 elapsed seconds. Callers that drive memory expiry (including stream apply after a payload) SHALL pass elapsed `now()`, not wall Unix seconds.
