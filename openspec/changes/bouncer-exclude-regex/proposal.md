...
- `ServeHTTP` matches `{host}/path` with no scheme, no port, no query. Host owner is Traefik/`net/http` `req.Host` (port-stripped with `net.SplitHostPort` when that succeeds). Path owner is `req.URL.Path` (decoded; empty Path treated as `/`). Do not use AppSec forwarded Host/URI. Do not call `captcha.RequestDomain`.
...
- `core_plugin_middleware_bouncer`: request-policy host+path exclude regexes skip that CrowdSec leg independently; match string reuses `req.Host` and `req.URL.Path`.
