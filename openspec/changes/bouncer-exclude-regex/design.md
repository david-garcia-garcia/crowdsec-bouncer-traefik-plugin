...
- Skip the named leg using host + `://` + path from `req.Host` and `req.URL.Path`.
...
3. Match string owner in `pkg/bouncer`: `excludeMatchString(httpReq *http.Request) string`. Host = `req.Host`; if `net.SplitHostPort` succeeds, use that host (IPv6 with port loses brackets; bare `[::1]` stays as Host wrote it). Path = `req.URL.Path`; nil URL or empty Path → `/`. Concatenate host + `://` + path (`example.com:///health`). Do not strip the path's leading slash. Alternative: AppSec forwarded Host/URI — rejected; port and query may remain; not this owner. Alternative: `captcha.RequestDomain` — rejected; display fallback. Alternative: `GetRemoteIP` — rejected; that owner is client address. Alternative: host + path without `://` (`example.com/health`) — rejected; requester answered host + `://` + path.
