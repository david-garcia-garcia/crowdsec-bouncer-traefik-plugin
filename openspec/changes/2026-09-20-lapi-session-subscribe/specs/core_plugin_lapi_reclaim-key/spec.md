...
**Reason**: Stream Open key no longer includes Redis store parameters. A Redis YAML change during grace Wakes the same session and keeps the live store.
**Migration**: Use ADDED requirement "Sleeping stream New Wakes the same session".
