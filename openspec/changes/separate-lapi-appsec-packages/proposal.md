...
- Rename `pkg/crowdsecconnection` to `pkg/lapi`. The reclaim type is `lapi.Client`. Drop exported `CrowdsecConnection`.
...
- `core_plugin_lapi_connection`: `pkg/lapi` owns the LAPI/CAPI reclaim Client (decisions, stream, cache, metrics). No AppSec.
...
- `core_plugin_appsec_failure-action`: AppSec fallback stays per-router; shared backend is `lapi.Client`, not a mixed type.
- `core_plugin_lapi_failure-action`: shared LAPI fallback is on `lapi.Client`.
