...
- [x] 2.1 Rename `pkg/crowdsecconnection` → `pkg/lapi`; type `Connection`; drop AppSec fields, client, and `AppsecQuery`
...
- [x] 3.2 Bouncer holds `lapiClient *lapi.Connection` and `appsecClient *appsec.Client`; `Query` on pass path; store `crowdsecMode` on Bouncer
