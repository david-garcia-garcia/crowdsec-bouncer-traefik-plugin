...
- Skip LookupRemediation and LiveLookup only for header `b`. Header `c` still looks up; a ban wins and WARN.
...
2. Read `req.Header.Get` after trusted skip. `b` remediates before lookup. `c` is merged after lookup via PreferRemediation semantics (ban wins). Alternative: skip lookup for `c` — rejected; a CrowdSec ban must supersede a neighbor captcha.
...
- [Header `c` must not hide a CrowdSec ban] → Mitigation: `c` still looks up; ban wins and WARN `ServeHTTP:forcedCaptchaSuperseded`.
