...
**Reason**: `httpTimeoutSeconds` and `EffectiveHTTPTimeoutSeconds` are deleted. Each backend owns its timeout. Nothing inherits.
**Migration**: Set `lapiHttpTimeoutSeconds`, `appsecHttpTimeoutSeconds`, and `bouncerCaptchaSiteverifyHttpTimeoutSeconds` independently (each default 10, MUST be `>= 1`). An operator who had `httpTimeoutSeconds: 2` and left per-leg overrides at 0 sets the LAPI and AppSec knobs to 2. No alias.
