...
- `core_plugin_appsec_client`: drain every non-nil AppSec response (including 502/503/504); body limit `0` is unlimited; rebuild outbound `Content-Length` from the bytes sent.
...
- README `CrowdsecAppsecBodyLimit` (`0` = unlimited).
