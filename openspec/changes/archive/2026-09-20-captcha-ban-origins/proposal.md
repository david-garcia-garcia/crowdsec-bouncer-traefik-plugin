...
CrowdSec CAPI and console-subscribed lists always deliver decision type `ban`. Console and `profiles.yaml` cannot change that type, so a visitor on a shared blocklist gets a hard 403. Upstream https://github.com/maxlerebourg/crowdsec-bouncer-traefik-plugin/pull/369 adds `CaptchaBanOrigins` matching raw `decision.Origin`. This fork names the Config field `BanToCaptchaOrigins`. This fork already stores `MetricsOrigin` (`lists:<scenario>`), so operators can remap one list without remapping every list.

## What Changes

- Add public Config `BanToCaptchaOrigins` (`banToCaptchaOrigins`), default empty (no-op).
...
- `core_plugin_lapi_ban-to-captcha-origins`: store captcha instead of ban for configured metrics origins, including per-list `lists:<name>`.
...
- `pkg/lapi/client.go`, `pkg/lapi/client_decisions.go`, `pkg/lapi/client_stream.go`, new `pkg/lapi/ban_to_captcha_origins.go`
