...
- Skip the named leg using `host://path` from `req.Host` and `req.URL.Path` with one leading `/` removed.
...
3. Match string owner in `pkg/bouncer`: `excludeMatchString(httpReq *http.Request) string`. Host = `req.Host`; if `net.SplitHostPort` succeeds, use that host (IPv6 with port loses brackets; bare `[::1]` stays as Host wrote it). Path = `req.URL.Path` with one leading `/` removed; nil URL, empty Path, or `/` → `host://`. Concatenate host + `://` + that path (`example.com://health`). Alternative: AppSec forwarded Host/URI — rejected; port and query may remain; not this owner. Alternative: `captcha.RequestDomain` — rejected; display fallback. Alternative: `GetRemoteIP` — rejected; that owner is client address. Alternative: host + path without `://` (`example.com/health`) — rejected. Alternative: keep the path's leading slash after `://` — rejected; requester answered `host://path` with exactly two slashes in `://`.
...
None — explore rows stand; the match-string row is resolved by the requester.
