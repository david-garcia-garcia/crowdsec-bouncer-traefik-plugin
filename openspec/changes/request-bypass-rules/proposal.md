...
- Authoring: optional `method`, `path`, `headers`, `cookies`. Omit = any. Set fields AND. List OR, first match wins. Method is Go RE2 against `req.Method` (optional leading `!` negates). Path is unanchored RE2 on `req.URL.Path`.
...
- `core_plugin_middleware_bouncer`: per-leg bypass lists replace host+path exclude strings; match is method/path/headers/cookies on the request; same ServeHTTP skip sites.
