...
1. Package `pkg/httprule`. Authoring type `Rule` with json `method`, `path`, `headers`, `cookies`. Compiled `Set` with `Match(*http.Request) bool`. Alternative: `pkg/bypassrule` — rejected; ties a later utilities move to this plugin's knob. Alternative: keep match helpers on `pkg/bouncer` — rejected; Desired is its own package with no plugin imports.
2. `httprule.New([]Rule) (*Set, error)` is the compile owner. Reject invalid RE2 (path, method, header, cookie) and fully empty rules. Fully empty = path, headers, and cookies all absent AND method is any (omitted, empty after trim, or a match-everything pattern such as `.*`). A method-only rule is valid. `!` with an empty pattern and `!!` are invalid syntax, not the fully-empty case. `ValidateParams` calls `New` and wraps the error with `BouncerAppsecBypassRules` / `BouncerLapiBypassRules`. `bouncer.New` calls it again to store `*httprule.Set` (same split as dest `CompileExcludeRegex`). Alternative: compile only in `bouncer.New` — rejected; constructor gate is `ValidateParams`. Alternative: put compiled regexes on `Config` — rejected; Config is the Traefik public struct.
