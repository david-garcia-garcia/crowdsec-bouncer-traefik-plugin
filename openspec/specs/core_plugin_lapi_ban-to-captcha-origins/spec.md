...
Maps selected LAPI `ban` origins to stored captcha kind using public Config `BanToCaptchaOrigins` and `MetricsOrigin`, including per-list `lists:<name>` matching.
