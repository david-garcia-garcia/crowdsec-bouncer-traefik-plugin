...
- **WHEN** two live middlewares reclaim the same `lapi.Client` and set different `bouncerLiveTtlSeconds`
...
When `lapiEnabled` is true and this middleware Opens LAPI, `New` SHALL fail if a DecisionStore for that LAPI session already exists with a different `createdBy` instance name (`core_plugin_lapi_reclaim-key`). Same instance name on many Openers MUST still share. Failed `New` SHALL still release bindCtx.
...
- **AND** if `bouncerRemediationHeader` is `X-Remediation` the response header value is `error:client-disconnected`
