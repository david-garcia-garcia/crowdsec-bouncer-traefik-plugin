...
House rule for how an in-repo Go test captures `slog` output it will assert on, so a test read cannot race a log write from a goroutine of the code under test.
