...
Maps selected LAPI decision types to a weaker kind at Bouncer apply using public Config `BouncerDecisionRemap` and `MetricsOrigin`, including per-list `lists:<name>` matching. LAPI and the DecisionStore keep the original kind.

## Requirements

### Requirement: Empty BouncerDecisionRemap leaves kinds unchanged
Public Config `BouncerDecisionRemap` (`json:"bouncerDecisionRemap"`) SHALL default to empty. When the Bouncer table is empty, a LAPI decision type `ban` SHALL store and apply as kind `t` (`BannedValue`) and type `captcha` SHALL store and apply as kind `c` (`CaptchaValue`). `ValidateParams` MUST NOT reject unknown origin tokens and MUST NOT require `bouncerCaptchaProvider` for this table. Blank origin keys SHALL be dropped when copied onto the Bouncer.

#### Scenario: Default config still bans CAPI
- **WHEN** `bouncerDecisionRemap` is empty or omitted and LAPI returns type `ban` origin `CAPI`
...
- **WHEN** `bouncerDecisionRemap` is `{CAPI: {ban: captcha}}` and a stream `ban` has origin `CAPI`
- **THEN** the Ip slot kind is ban (`t`)
- **AND** ServeHTTP remediates as captcha (`c`)

#### Scenario: Unlisted origin stays ban
- **WHEN** `bouncerDecisionRemap` is `{CAPI: {ban: captcha}, lists: {ban: captcha}}` and a `ban` has origin `cscli`
- **THEN** the stored kind is ban (`t`)
- **AND** ServeHTTP remediates as ban

#### Scenario: Captcha pass keeps the stored captcha
- **WHEN** `bouncerDecisionRemap` is `{crowdsec: {captcha: pass}}` and a stream `captcha` has origin `crowdsec`
...
- **WHEN** `bouncerDecisionRemap` is `{CAPI: {ban: captcha, captcha: pass}}` and the stored kind is ban (`t`) origin `CAPI`
- **THEN** ServeHTTP remediates as captcha (`c`)

#### Scenario: Invalid edge fails validation
- **WHEN** `bouncerDecisionRemap` is `{CAPI: {captcha: ban}}`
...
- **WHEN** `bouncerDecisionRemap` is `{lists:firehol_level1: {ban: captcha}}` and a stored `ban` has origin `lists:firehol_level1`
- **THEN** ServeHTTP remediates as captcha (`c`)
...
- **WHEN** `bouncerDecisionRemap` is `{lists: {ban: captcha}}` and a stored `ban` has origin `lists:tor-exit`
- **THEN** ServeHTTP remediates as captcha (`c`)

### Requirement: Live strongest pick uses LAPI type
`queryLiveDecisions` SHALL pick among LAPI decisions using the LAPI type: the first ban wins, else the first captcha. It MUST NOT apply `BouncerDecisionRemap` when picking or when writing the live cache.
...
### Requirement: BouncerDecisionRemap is per Bouncer
`bouncer.New` SHALL copy `BouncerDecisionRemap` onto that Bouncer. The table MUST NOT appear in stream `SessionKey` or live/none `Key`. Two Bouncers sharing one Client MAY disagree. Stream apply MUST NOT use the first `New` table.

#### Scenario: Two routers disagree
- **WHEN** two stream `New` calls share LAPI URL, key, and Redis and disagree only on `bouncerDecisionRemap`
