...
When `crowdsecAppsecEnabled` is true, `New` SHALL reclaim an `appsec.Client` with `reclaim.OpenWithGrace` and a 30s grace. The reclaim key SHALL be derived from AppSec scheme, host, path, key, TLS, body limit, and the effective AppSec HTTP timeout (`crowdsecAppsecHttpTimeoutSeconds` when greater than zero, otherwise `httpTimeoutSeconds`). Middleware name, `next`, templates, trusted IPs, Enabled, and LAPI fields MUST NOT be in that key. The create func SHALL return `*reclaim.Wrapped`. `Close` SHALL release idle AppSec HTTP connections.
...
- **WHEN** one middleware inherits a 10 second AppSec timeout from `httpTimeoutSeconds` and another sets `crowdsecAppsecHttpTimeoutSeconds` to 10 with the same AppSec URL, key, and TLS
...
Public config `crowdsecAppsecHttpTimeoutSeconds` SHALL be an integer second timeout for the AppSec HTTP client. Zero or omit SHALL use `httpTimeoutSeconds`. A value greater than zero SHALL be that many seconds. A negative value SHALL be rejected at ValidateParams.

#### Scenario: Omit inherits HTTPTimeoutSeconds
- **WHEN** the operator omits `crowdsecAppsecHttpTimeoutSeconds` and sets `httpTimeoutSeconds` to 10
- **THEN** AppSec HTTP calls use a 10 second timeout

#### Scenario: Explicit seconds override HTTPTimeoutSeconds
- **WHEN** the operator sets `crowdsecAppsecHttpTimeoutSeconds` to 1 and `httpTimeoutSeconds` to 10
- **THEN** AppSec HTTP calls use a 1 second timeout
- **AND** LAPI HTTP calls still use `httpTimeoutSeconds` or `crowdsecLapiHttpTimeoutSeconds`

#### Scenario: Negative seconds is rejected
- **WHEN** the operator sets `crowdsecAppsecHttpTimeoutSeconds` to -1
