...
In `lapiMode: alone`, `ValidateParams` SHALL still validate captcha site/secret keys when `captchaEnabled` is true, the captcha template when `captchaEnabled` is true (empty `CaptchaFilePath` or a `GetTemplate` failure SHALL fail), the ban template when that path is set, and log level / writable log file path. It MAY skip LAPI URL, LAPI key, and LAPI TLS checks after CAPI credential validation. Empty site or secret after file-then-field lookup SHALL fail even when `CaptchaGateSecret` is set. When `appsecEnabled` is true, it SHALL still validate AppSec URL, AppSec key, and AppSec HTTPS CA.
...
- **WHEN** `captchaEnabled` is true, `captchaProvider` is set, site is non-empty, and secret resolves empty
