...
### Requirement: BouncerLapiFailureAction is the public LAPI fallback
Public config `bouncerLapiFailureAction` SHALL be one of `passthrough`, `ban`, or `captcha`. The default SHALL be `ban`. `captcha` SHALL be rejected at ValidateParams unless a captcha provider is configured. Empty SHALL be treated as `ban` (same as omit). Unknown values SHALL be rejected.
...
- **WHEN** `bouncerLapiFailureAction` is `captcha` and no captcha provider is set
- **THEN** plugin initialization fails validation

### Requirement: LapiUpdateMaxFailure remains the stream unhealthy counter
...
### Requirement: Live LAPI error uses BouncerLapiFailureAction
...
### Requirement: Stream unhealthy miss uses BouncerLapiFailureAction
...
### Requirement: BouncerLapiFailureAction is per-router on Bouncer
