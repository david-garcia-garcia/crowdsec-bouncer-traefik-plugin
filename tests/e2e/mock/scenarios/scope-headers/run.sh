...
body() {
  ...
  wait_for_status "$URL" 403 45 -H "X-Forwarded-For: 203.0.113.10" -H "CF-IPCountry: fr"
  ...
  wait_for_status "$URL" 200 45 -H "X-Forwarded-For: 203.0.113.10" -H "CF-IPCountry: FR"

  echo "[$SCENARIO] AS 13335 ban (header AS13335) -> 403"
  lapi_add_scope_decision AS 13335 ban 5m
  wait_for_status "$URL" 403 45 -H "X-Forwarded-For: 203.0.113.10" -H "CF-ASN: AS13335"
  ...
  wait_for_status "$URL" 403 45 -H "X-Forwarded-For: 203.0.113.10" -H "X-User: alice"
  ...
  wait_for_status "$URL" 403 45 -H "X-Forwarded-For: 10.1.2.3"
