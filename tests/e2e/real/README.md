...
Country matching on `/scope-none` and `/scope-stream` chains [traefik-geoblock](https://github.com/david-garcia-garcia/traefik-geoblock) in enrich mode, then the bouncer. `Test-Integration.ps1` clones geoblock `v1.2.0` into `.geoblock/` (gitignored) before `docker compose up`. Nested plugin maps live in `dynamic/dynamic-scopes.yml` (watched file-provider directory). Instance-severance reloads write `dynamic/instance-severance.yml`.
