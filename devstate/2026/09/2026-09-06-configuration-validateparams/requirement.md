...
## Current (code)
- `pkg/configuration/configuration.go:339` — AppSec URL validated with `LapiScheme`, not `AppsecScheme` (nor LAPI fallback).
- `pkg/configuration/configuration.go:378-383` — TLS validation runs only when `LapiScheme == HTTPS`; no AppSec HTTPS branch.
- `pkg/configuration/configuration.go:438-452` — `validateParamsTLS` parses only `LapiTlsCa`; AppSec CA fields ignored.
...
## Desired
- Validate AppSec URL using effective scheme (`AppsecScheme` if set, else `LapiScheme`).
...
- Add focused tests: custom captcha provider missing fields; AppSec `captcha` failure action without provider; `appsec` mode without LAPI key; alone mode with captcha failure-action and missing secrets/templates; `GetTemplate` error paths; `validateURL` bad host/path; `BouncerRemediationStatusCode` 99/600; `LapiUpdateMaxFailure: -1`.
...
## Unknowns
- Whether AppSec client cert fields (`AppsecTlsCert*`) need the same startup parse as LAPI bouncer certs, or CA-only parity is enough for this ticket.
