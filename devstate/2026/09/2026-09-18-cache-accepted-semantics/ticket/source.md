...
1. Read-your-writes is rejected as overengineering. Forget it. nextReader stays replica round-robin when RedisCacheReadHosts is set; when empty, reads are the writer. Replica lag after a primary Set (stream miss → allow) is accepted. Do not retry Get on the writer on miss. Do not keep a local set of written keys.
2. TTL <= 0 / Redis SET EX 0 is accepted. storeStreamDecision uses int64(duration.Seconds()); sub-second CrowdSec durations become 0; SimpleRedis always sends SET EX <n>; Redis rejects EX 0; key is not stored; error is logged. liveCacheTTL already substitutes defaultDecisionSeconds when durationSecond<=0 — do not change that. Do not clamp stream TTL. Do not make localCache match Redis.
...
- README RedisCacheReadHosts already mentions replica outage; only add a lag/stale-read sentence if explore finds the knob text still implies reads are consistent with the last write.
