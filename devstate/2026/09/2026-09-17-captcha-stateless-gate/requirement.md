...
- `pkg/configuration/configuration.go` — `BouncerCaptchaGracePeriodSeconds` default 1800; `BouncerCaptchaSecretKey` is provider siteverify only; no bind-IP/cookie-only knob or dedicated HMAC secret (`112-123`, `193-194`).
...
## Desired
- After successful provider verify, set an HttpOnly signed cookie (HMAC with dedicated plugin secret, not `BouncerCaptchaSecretKey`): payload includes version, `issued_at`, optional bind-IP flag, normalized IP when bound; validate with constant-time HMAC, expiry against `BouncerCaptchaGracePeriodSeconds`, optional IP match via same normalization as `GetRemoteIP`.
