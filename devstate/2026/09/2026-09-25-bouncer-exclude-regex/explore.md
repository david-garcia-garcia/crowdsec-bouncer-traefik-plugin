...
- Q: Exact `{host}/path` build from `*http.Request` (which host field, path encoding, leading slash)?
  ...
  Decision: resolved — answered by the requester. Host is `req.Host` after `net.SplitHostPort` when that succeeds (IPv6 with port loses brackets; bare `[::1]` stays as Host wrote it). Path is `req.URL.Path` (decoded, no query) with one leading `/` removed. Empty Path or `/` → `host://`. Concatenate host + `://` + that path (`example.com` + `/health` → `example.com://health`; `/v2/blobs` → `example.com://v2/blobs`). Do not lowercase. Do not use `URL.String()`, `RequestURI`, or `EscapedPath`.
  By: implement
