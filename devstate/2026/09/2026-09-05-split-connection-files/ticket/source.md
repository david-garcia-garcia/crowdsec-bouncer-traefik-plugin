...
Keep the same package. Same exported API. No new packages (`pkg/appsec` is out of scope). No behavior change. Do not move GetTLSConfigCrowdsec (sibling ticket). Do not change identity, LapiScopeHeaders, cache constants, pkg/ip, configuration.go layout, or Prepare/plugin mutation.
