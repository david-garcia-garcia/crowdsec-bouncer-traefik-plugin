...
- `scopes=` is `decisionscope.StreamScopeList(c.lapiScopeHeaders)` (`pkg/decisionscope/lookup.go`). Headers live on the connection field set in `New`, not in the reclaim key. First `New` for a shared hash wins the scopes query.
...
- No `identity_test.go` on dest. In-flight PR #18 (`2026-09-05-scope-headers-identity`) puts `LapiScopeHeaders` on identity so different scopes do not first-win; it does not treat same-key extra identity fields as a config error.

## Desired
- Stream session is an explicit unique in-process resource: LAPI scheme/host/path + lapiKey (CAPI machine+password in alone) + `scopes=` from `NormalizeLapiScopeHeaders`. Scopes belong in that key.
