...
Identity includes: mode, LAPI/CAPI, LapiUpdateIntervalSeconds, LapiMetricsIntervalSeconds, LapiUpdateMaxFailure, LapiFailureAction, LapiStreamStartupBlock, BouncerLiveTtlSeconds, HTTPTimeoutSeconds, Redis*, AppSec client, TLS. Comment says middleware name, templates, trusted IPs, Enabled are out.
...
lapiScopeHeaders is on the connection and changes scopes=, but is not in identity. Opposite bug: two middlewares that should be different sessions can share one poller (first New wins scopes). E2e already papered that with BOUNCER_KEY_TRAEFIK_SCOPES.
...
Treat “set the same lapiMetricsIntervalSeconds on /trusted” as the product fix. That only makes the e2e hashes match.
Remove metrics from identity and stop. LapiUpdateIntervalSeconds (and Redis, timeout, failure action) split the same way.
...
Define session key: LAPI scheme/host/path + lapiKey (or CAPI machine+password in alone) + scopes= from NormalizeLapiScopeHeaders. Put that in identity (scopes must be in the key; they are not today).
...
Unit: two Key(cfg) with same LAPI key+mode+host, differ only LapiMetricsIntervalSeconds (and separately only LapiUpdateIntervalSeconds) — today they differ; after the fix, either same key or New fails. Pin the chosen behavior.
Unit: lapiScopeHeaders different ⇒ different session key (must not first-win).
