...
- `Prepare` (`connection.go`) resolves LAPI key (or CAPI host in `alone`) and, when AppSec key is empty, copies `LapiKey` into `AppsecKey`; AppSec scheme also falls back to LAPI scheme.
- `New` builds both TLS clients; `LapiMode == appsec` returns before cache/stream start so the same type is used as an AppSec-only connection (`connection.go`).
- `pkg/crowdsecconnection/connection_appsec.go` owns `AppsecQuery`, `AppsecResponse`, `AppsecPolicy`, `ErrFailureCaptcha`. `pkg/bouncer/bouncer.go` calls `b.conn.AppsecQuery` on the pass path when `appsecEnabled`.
...
- `lapiMode` enum mixes LAPI modes (`none`/`live`/`stream`/`alone`) with `appsec` (`pkg/configuration/configuration.go`). `appsecEnabled` is orthogonal and can run on stream/live.
...
- Renaming `lapiMode: appsec` or `appsecEnabled` (unless explore decides the mode value is in scope).
...
- How `lapiMode: appsec` (skip LAPI) sits next to `appsecEnabled` (run AppSec on a LAPI mode).
...
- Official AppSec protocol notes the bouncer API key is often the same as LAPI; this plugin already has a distinct `appsecKey` with LAPI fallback (`Prepare`).
