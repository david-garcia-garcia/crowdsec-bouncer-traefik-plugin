...
- `pkg/configuration/configuration.go:200` — default `BouncerForwardedTrustedIPs` is an empty slice (context only; out of scope to change here).
...
- `BouncerClientTrustedIPs` bypass after identity is chosen
...
## Unknowns
- Whether the RemoteAddr trusted-proxy check reuses the same `PoolStrategy.Checker` instance wired from `BouncerForwardedTrustedIPs` or needs a separate pool (ticket text uses trusted-hop pool; explore may confirm wiring in bouncer only reads config once).
...
- Default empty `BouncerForwardedTrustedIPs` plus header-present path accepts spoofed client IPs today.
