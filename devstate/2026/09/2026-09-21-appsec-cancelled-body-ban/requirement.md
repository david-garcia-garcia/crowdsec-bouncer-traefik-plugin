...
- `pkg/configuration/configuration.go`: default `CrowdsecAppsecBodyLimit` is `10485760`; `CrowdsecAppsecFailureAction` gates unreachable, AppSec 500, and `appsecQuery:unreadableBody dropped` via `resultForFailureAction`, not the `GetBody` error path (`pkg/configuration/configuration.go`, `pkg/appsec/query.go` `Query` / `newAppsecBodyRequest`).
...
## Unknowns
- Preferred fix shape on upstream (silent pass-through vs extending `CrowdsecAppsecFailureAction` vs new knob) — reporter offered either.
...
## Tensions
- #395 describes three legacy `*Block` bools; this fork on `master` uses `crowdsecAppsecFailureAction` instead, but **mid-stream `GetBody` failures still always ban** — passthrough on that path is not available today.
