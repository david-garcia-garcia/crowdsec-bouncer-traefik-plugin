...
## Problem
ValidateParams accepts `appsecEnabled: true` with an empty `AppsecHost`. `plugin.New` then starts. Every AppSec Query is unreachable; default AppSec failure action bans the site.
...
- `AppsecHost` is not in `validateParamsRequired`. `pkg/configuration/configuration.go`
- `New()` defaults host to `crowdsec:7422` and `BouncerAppsecFailureAction` to `ban`. `pkg/configuration/configuration.go`
...
## Desired
- When AppSec is enabled, reject an empty `appsecHost` and any AppSec URL that `http.NewRequest` accepts only because the host is missing.
...
- Changing LAPI `validateURL` or the already-required `LapiHost`
- Adding AppSec URL checks in `alone` mode (ValidateParams skips `validateLapiAndAppsecConnection` there)
- Changing default `BouncerAppsecFailureAction` or Query/Bouncer remediations
