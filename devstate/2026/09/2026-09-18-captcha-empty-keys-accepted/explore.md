...
`ValidateParams` always runs captcha credential/template checks when `CaptchaProvider` is set (`validateEnabledCaptchaSettings`), including `crowdsecMode: alone`. Alone only skips LAPI URL/key/TLS after CAPI file/field lookup. `plugin.go` `New` already returns `nil, err` on `ValidateParams` failure — no handler.
...
`validateCaptchaCredentials` discards those strings and returns only lookup errors. `CaptchaGateSecret` on the same path already rejects `""` after lookup. Site and secret do not.
...
GetVariable(gate) ── "" ──► error (today)
        │
        ▼
optional CaptchaFilePath template
```
...
Usage packets: `core_plugin_middleware.md` covers `New` / reclaim, not this empty-key rule. `core_plugin_middleware_captcha-gate.md` requires a gate secret when a provider is set; it does not say site/secret must be non-empty after lookup. That is enough to call the existing empty-check pattern (`CaptchaGateSecret`). No new packet this phase — the desired contract is not on DestBranch yet. No third-party research write: this is our `ValidateParams`, not provider HTTP or Traefik `New` lookup.

This work does not set or reconstruct client address, user, tenant, Host, or a trust hop.

## Decisions

1. **Owner** — fix `validateCaptchaCredentials`. Keep lookup errors. After a successful `GetVariable`, reject `""` for site and for secret, independently, site first. Same trigger as the gate secret: `CaptchaProvider != ""`, not “failure action is captcha”.
2. **Lookup** — reuse `GetVariable`. Do not add env.
3. **Messages** — same shape as the gate secret: `CaptchaSiteKey: cannot be empty when CaptchaProvider is set` (and the secret twin).
...
- Q: Reject empty site/secret only when a failure action is `captcha`, or whenever `captchaProvider` is set?
  Decision: resolved — whenever `CaptchaProvider` is set. Matches `CaptchaGateSecret` and the requirement text. Spec scenario is one case of that rule.
  By: explore
...
- Q: What error text, and in what order?
  Decision: assumed — first-fail, site then secret, same wording as `CaptchaGateSecret: cannot be empty when CaptchaProvider is set`.
  By: explore
