Developer review: needs changes — 2026-09-06T15:06:23Z
...
**Developers.** `cache.Client.Set` returns `error`; captcha `Validate(r, remoteIP)` sends `remoteip` on siteverify, gates 302 on grace cache write, and re-renders captcha (200) on retryable provider errors; `pkg/captcha/captcha_test.go` covers verify, ServeHTTP, and New paths; devdocs added `core_plugin_captcha_handler` and updated `core_cache_client` Set observability.
...
Local bug-hunt ticket → branch `2026-09-06-captcha-handler-hardening` → PR #28 → implement landed cache/config/captcha commits → codereview fixed test stub naming → devdocsimpact added captcha handler packet and cache Set usage.
...
| Reviewed head | fe1c1ef2 | Devdocs impact on pinned diff origin/master...HEAD |
...
### Technical review
Best possible solution: Minimal shared `Set` error seam plus captcha UX fixes match explore decisions; devdocs now cover captcha handler and cache write observability.
...
### Evidence
What I checked:
- Pinned diff `origin/master...HEAD` excluding devstate and `.cursor`
- Devdocs consume: no captcha packet; cache client missing Set error contract
- Produced `core_plugin_captcha_handler.md` and updated `core_cache_client.md`
