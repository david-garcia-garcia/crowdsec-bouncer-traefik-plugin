...
- Q: What is the vendor widget script URL and CSS class?
  Rank: additive asked — new Widget pairing this change creates; Desired names script URL and CSS class
  Decision: resolved — script https://cdn.eu-captcha.eu/verify.js, class eu-captcha. Source knowledge/research/ext_eucaptcha_widget/.
  By: explore

- Q: Does the vendor widget inject hidden field eu-captcha-response so stock captcha.html needs no extra field?
  Rank: additive asked — TokenField on the pairing this change creates; Out of scope Changing the stock challenge page
  Decision: resolved — yes. verify.js writes eu-captcha-response itself. Canonical samples are a form with only the eu-captcha div. Keep captcha.html. TokenField is eu-captcha-response. Source knowledge/research/ext_eucaptcha_widget/.
  By: explore

- Q: What are the exact JSON types, HTTP statuses, and error bodies for POST /v1/verify?
  Rank: additive asked — new verifier this change creates; Desired names the verify URL and JSON fields
