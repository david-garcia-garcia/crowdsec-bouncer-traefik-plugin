...
## Problem
Three leftovers on dest captcha: siteverify omits bouncer-resolved `remoteip`; transport and JSON-decode errors become bare HTTP 400; an empty `BouncerCaptchaFile` with a provider set is accepted and `Client.New` discards `GetTemplate` error.
...
- Default `BouncerCaptchaFile` is `/captcha.html`. Path: `pkg/configuration/configuration.go`.
- `GetTemplate("")` errors `no template file provided`. Path: `pkg/configuration/configuration.go`.
- `validateEnabledCaptchaSettings`: provider set + `BouncerCaptchaFile == ""` returns nil; non-empty path runs `GetTemplate` and fails on read/parse. Path: `pkg/configuration/configuration.go`.
- Test "Captcha LAPI action with provider" expects success with empty `BouncerCaptchaFile`. Several captcha tests blank the path to skip template load. Path: `pkg/configuration/zzz_configuration_test.go`.
...
3. When `BouncerCaptchaProvider` is set, `ValidateParams` fails if `BouncerCaptchaFile` is empty or `GetTemplate` fails. `Client.New` returns the `GetTemplate` error. No bundled template.
...
- Captcha and configuration tests that blank `BouncerCaptchaFile` or assert 400 on provider errors
...
- Gate cookie format, Secure, `BouncerCaptchaGateSecret`, bind-IP
...
- Tests that blank `BouncerCaptchaFile` to skip `GetTemplate` will need a real file once empty path is rejected.

## Tensions
- Config spec says validate templates "when paths are set"; ticket requires fail on empty path when provider is set.
- "Captcha LAPI action with provider" and custom-challenge tests expect success with empty `BouncerCaptchaFile`.
