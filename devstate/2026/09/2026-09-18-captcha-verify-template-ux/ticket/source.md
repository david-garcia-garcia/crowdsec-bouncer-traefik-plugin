Captcha siteverify remoteIP, retryable provider errors, and required template when BouncerCaptchaProvider is set.
...
3) When BouncerCaptchaProvider is set, require a loadable captcha template: ValidateParams fails if BouncerCaptchaFile is empty or GetTemplate fails. Client.New must return GetTemplate error (today it discards with _). Default config BouncerCaptchaFile is /captcha.html — empty path is the gap. Do not invent a bundled template.
...
Bound: do not change cache.Set API. Do not write remoteIP+_captcha. Do not change gate cookie format, Secure, or BouncerCaptchaGateSecret. Do not touch LAPI, AppSec, Range, Redis TTL, or HTML-path deprecation removal (#100). Do not import traefik-modsecurity.
