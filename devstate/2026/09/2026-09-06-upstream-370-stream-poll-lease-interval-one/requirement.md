...
Upstream #370: with `lapiUpdateIntervalSeconds: 1`, the stream poll lease key (`updated`) is never stored because lease TTL is `updateInterval - 1` (= 0). Zero TTL is a no-op in the local cache and fails on Redis, so multi-instance deployments poll LAPI on every tick instead of one poll per interval. Our fork already floors lease duration at 1 second, but no test proves the fix for interval 1.
...
- `pkg/configuration/configuration.go:502-511` — `LapiUpdateIntervalSeconds` validated `>= 1` via `requiredInt1`.
