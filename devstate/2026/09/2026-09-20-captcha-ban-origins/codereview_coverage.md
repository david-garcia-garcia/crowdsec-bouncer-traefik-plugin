...
**Tests in diff / still relevant:** `pkg/lapi/zzz_ban_to_captcha_origins_test.go` (match rules, copy trim, `remediationKindForOrigin`, `streamPutItem`, `strongestLiveDecision`); `tests/e2e/mock/scenarios/captcha-ban-origins/run.sh` (stream mode: listed CAPI / `lists:firehol_level1` → captcha page, tor-exit / cscli → 403).
...
2. [hard] Critical path untested — `pkg/lapi/client_decisions.go:115-116` — `queryLiveDecisions` sets cached `liveResult.kind` via `c.remediationKindForOrigin(picked.Type, origin)`; `TestStrongestLiveDecisionBanToCaptchaOrigins` only exercises `strongestLiveDecision` plus a separate `remediationKindForOrigin` call, not `queryLiveDecisions` — reverting the kind line to `RemediationValue(picked.Type)` stays green for listed-only live picks
