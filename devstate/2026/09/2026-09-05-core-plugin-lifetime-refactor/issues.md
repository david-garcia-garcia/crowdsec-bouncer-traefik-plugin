...
- [ ] take small  `pkg/cache` process-wide `ttl_map` (`cache.go` var `cache`) → per-Client map owned by Connection
  Why: human: no shared in-memory store so Connection is testable. Same package; captcha already takes `*cache.Client`.
