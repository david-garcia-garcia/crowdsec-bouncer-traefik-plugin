...
Owner of the **client address**. `ForwardedHeadersInsecure` or `RemoteAddr` in `ForwardedHeadersTrustedIPs` gates whether this plugin reads the custom **IP** header. That list can differ from Traefik’s entrypoint `forwardedHeaders.trustedIPs`. It does not own scheme.
...
- Parse: `http.CanonicalHeaderKey` / `Header.Get("X-Forwarded-Proto")`, trim space, `EqualFold` to `https` on that whole value. No comma split (Traefik writes a single token and compares with `== "https"`). Do not read `Forwarded`, `Front-End-Https`, `X-Forwarded-Protocol`, `X-Scheme`, or `ForwardedHeadersCustomName`. Do not treat `wss` as cookie Secure. Do not use `r.URL.Scheme`.
