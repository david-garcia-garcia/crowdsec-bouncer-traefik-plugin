...
On this tree the public knob is `bouncerAppsecFailureAction` (`passthrough` | `ban` | `captcha`). `Client.Query` classifies transport errors and `isReverseProxyError` (502/503/504) as unreachable and runs `resultForFailureAction`. HTTP 500 is a separate log (`appsecQuery:failure`) with the same action mapper.
...
- E2E mock: `/foo/502` with default `ban` → 403 (`tests/e2e/mock/scenarios/appsec/`). Scenario YAML is locked to `bouncerAppsecFailureAction: ban`. Mocklapi returns 502 when the forwarded URI contains `"502"`; no 503/504 paths.
...
Usage packet `knowledge/devdocs/core_plugin_appsec.md` already says 500, unreachable, and unreadable body share `bouncerAppsecFailureAction`. Research `ext_crowdsec_bouncers_failure-action/` and `ext_crowdsec_appsec_protocol/` already document Traefik treating 502/503/504 as unreachable. No new research write. Spec leaf `core_plugin_appsec_failure-action` already requires 502/503/504 as unreachable and passthrough→`next`. Propose folds a named HTTP-status scenario onto that leaf rather than a new spec.
