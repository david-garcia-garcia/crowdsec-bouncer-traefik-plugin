...
- `pkg/configuration/configuration.go` — public key is `bouncerAppsecFailureAction` (`passthrough` | `ban` | `captcha`); legacy `crowdsecAppsecUnreachableBlock` removed.
...
## Desired
Add tests (unit and/or e2e) that prove when AppSec returns HTTP 502, 503, or 504, `bouncerAppsecFailureAction: passthrough` allows the request (same as transport unreachable). Do not change product behavior unless a test cannot be honest without a one-line correctness fix.
...
## Unknowns
- Whether e2e mock can configure `bouncerAppsecFailureAction: passthrough` per scenario without new fixture wiring.
