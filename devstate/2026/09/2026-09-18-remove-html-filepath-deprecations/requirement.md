...
## Problem
Deprecated `banHtmlFilePath` and `captchaHtmlFilePath` still exist as Config fields and `plugin.New` aliases. Ban copies only when `BouncerBanFile` is empty; captcha copies whenever the old key is non-empty and overwrites `BouncerCaptchaFile`. The ticket wants those settings gone, with no compatibility alias.
...
- `configuration.New` defaults `BouncerBanFile` to `""` and `BouncerCaptchaFile` to `/captcha.html`. It does not set the HTML-path fields. `pkg/configuration/configuration.go`
- `plugin.New` copies `BanHTMLFilePath` onto `BouncerBanFile` only when `BouncerBanFile` is empty. `plugin.go`
- `plugin.New` copies `CaptchaHTMLFilePath` onto `BouncerCaptchaFile` whenever `CaptchaHTMLFilePath` is non-empty (overwrites). `plugin.go`
- Template load and Content-Type inference read `BouncerBanFile` / `BouncerCaptchaFile` only. `pkg/configuration/configuration.go`
...
- Mock custom-ban scenario already sets `bouncerBanFile`. `tests/e2e/mock/scenarios/custom-ban-page/dynamic.yml`
...
- In-tree custom-ban example already uses `bouncerBanFile`. `examples/custom-ban-page/docker-compose.yml`
...
- Operators configure `bouncerBanFile` and `bouncerCaptchaFile` only. YAML that still sets the old keys must not be copied onto the current fields.
- Update in-tree e2e YAML, examples, live OpenSpec, README/docs, and tests that still name the old keys to `bouncerBanFile` / `bouncerCaptchaFile`.
...
## Out of scope
- Template loading, Content-Type inference, and defaults for `BouncerBanFile` / `BouncerCaptchaFile`
- Other config knobs
