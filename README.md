...
RE2 pattern matched against `host://path` (no scheme, no port, no query). Examples: Host `example.com` Path `/health` → `example.com://health`; Host `example.com:443` Path `/health` → `example.com://health`; query ignored → `example.com://health`; Path `/v2/blobs` → `example.com://v2/blobs`; Path `/` or empty → `example.com://`. A match skips the AppSec query on the pass path. Empty after trim is off. The match is unanchored (`MatchString` reports whether the string contains any match); write `^...$` to match the whole string. Case-sensitive unless the pattern includes `(?i)`. Invalid pattern fails `New`. Host is the request `Host` with the port stripped (`net.SplitHostPort` when that succeeds; an IPv6 host with a port loses brackets). Path is the decoded URL path with one leading `/` removed.
