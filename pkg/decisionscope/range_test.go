...
func TestLookupCachedRemediationHeaderScope(t *testing.T) {
	client := newTestDecisionCache()
	if err := client.Set(HeaderScopeKey(ScopeCountry, "FR"), cache.BannedValue, 60); err != nil {
...
func TestLookupCachedRemediationOriginSuffix(t *testing.T) {
	client := newTestDecisionCache()
	if err := client.Set("203.0.113.10", cache.RemediationWithOrigin(cache.BannedValue, "crowdsec"), 60); err != nil {
		t.Fatalf("Set() error = %v", err)
	}
