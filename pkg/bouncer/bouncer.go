...
type Bouncer struct {
	...
	lapiMode                     string
	lapiScopeHeaders             map[string]string // CrowdSec header scope → request header
	forcedDecisionHeader         string            // bouncerDecisionHeader; empty = off
	enabled                      bool
	forwardedCustomHeader        string
	bouncerForwardedInsecure     bool
	...
	bouncerRemediationStatusCode int
	serverPoolStrategy           *ip.PoolStrategy
	template                     *template.Template
	traceCustomHeader            string
	bouncerDecisionRemap         map[string]map[string]string // per-router apply; LAPI/store keep original kinds
}

// remediationHeaderClientDisconnected is the BouncerRemediationHeader value when the client
...
func New(next http.Handler, name string, config *configuration.Config, lapiClient *lapi.Client, appsecClient *appsec.Client, log *slog.Logger) (http.Handler, error) {
	serverChecker, _ := ip.NewChecker(log, config.BouncerForwardedTrustedIPs)
	clientChecker, _ := ip.NewChecker(log, config.BouncerClientTrustedIPs)
	forwardedCustomHeader := config.BouncerForwardedHeader
	if config.BouncerForwardedInsecure && forwardedCustomHeader == "X-Forwarded-For" {
		forwardedCustomHeader = "X-Real-Ip"
	}
	if config.BouncerForwardedInsecure {
		log.Info("BouncerForwardedInsecure enabled", "header", forwardedCustomHeader)
	}

	var banTemplate *template.Template
	var banTemplateContentType string
	if config.BouncerBanFile != "" {
		banTemplate, banTemplateContentType, _ = configuration.GetTemplate(config.BouncerBanFile)
	}
	...
	routeHandler := &Bouncer{
		...
		bouncerForwardedInsecure:     config.BouncerForwardedInsecure,
		lapiClient:                   lapiClient,
		lapiEnabled:                  config.LapiEnabled,
		lapiInstanceName:             configuration.NamedLapiInstance(config, name),
		...
		name:                         name,
		next:                         next,
		remediationCustomHeader:      config.BouncerRemediationHeader,
		bouncerRemediationStatusCode: config.BouncerRemediationStatusCode,
		serverPoolStrategy:           &ip.PoolStrategy{Checker: serverChecker},
		template:                     template.New("CrowdsecBouncer").Delims("[[", "]]"),
		traceCustomHeader:            config.BouncerTraceHeader,
		bouncerDecisionRemap:         copyOriginBasedDecisionRemap(config.BouncerDecisionRemap),
	}
	// AppSec-only (`lapiEnabled` false) has no LAPI decisions to remediate, but
	// bouncerAppsecFailureAction: captcha still serves a challenge through this client
	// (core_plugin_appsec_failure-action), and handleRemediationServeHTTP bans whenever
	// that client is not valid.
	...
	bouncerCaptchaGateSecret, _ := configuration.GetVariable(config, "BouncerCaptchaGateSecret")
	err := routeHandler.captchaClient.New(
		...
		bouncerCaptchaGateSecret,
		config.BouncerCaptchaGateBindIP,
		config.BouncerRemediationHeader,
		config.BouncerCaptchaFile,
...
// LapiClient is the published or injected LAPI backend this route uses.
func (b *Bouncer) LapiClient() *lapi.Client {
	return b.publishedLAPI()
}

// SameLapiClient reports whether two routes share one LAPI client pointer.
func (b *Bouncer) SameLapiClient(other *Bouncer) bool {
	return other != nil && b.publishedLAPI() != nil && b.publishedLAPI() == other.publishedLAPI()
}

// publishedLAPI Peeks the named slot, then falls back to the client New received.
func (b *Bouncer) publishedLAPI() *lapi.Client {
	if b.lapiInstanceName != "" {
		if client := instance.PeekLAPI(b.lapiInstanceName); client != nil {
			return client
		}
	}
	return b.lapiClient
...
// publishedAppsec Peeks the named slot, then falls back to the client New received.
func (b *Bouncer) publishedAppsec() *appsec.Client {
	if b.appsecInstanceName != "" {
		if client := instance.PeekAppsec(b.appsecInstanceName); client != nil {
			return client
		}
	}
	return b.appsecClient
}
...
// lapiEnabled false: skip LAPI; pass path (AppSec if enabled).
//
...
func (b *Bouncer) ServeHTTP(rw http.ResponseWriter, httpReq *http.Request) {
	...
	remoteIP, ipAddr, err := ip.GetRemoteIP(httpReq, b.serverPoolStrategy, b.forwardedCustomHeader, b.bouncerForwardedInsecure)
	req := clientRequest{
		Request:  httpReq,
		ipAddr:   ipAddr,
	...
	lapiClient := b.publishedLAPI()
	if lapiClient == nil {
		b.applyLapiFailureAction(rw, req, configuration.ReasonLAPI, lapi.OriginPluginLapiFailure)
		return
	}
	lapiMode := lapiClient.LapiMode()

	// Mapped scope headers for this request. Missing headers are omitted.
	scopes := decisionscope.RequestScopeValues(b.lapiScopeHeaders, req.Request)

	// live, stream, and alone consult the cache.
	if lapiMode == configuration.LiveMode || lapiMode == configuration.StreamMode || lapiMode == configuration.AloneMode {
	...
	if lapiMode == configuration.StreamMode || lapiMode == configuration.AloneMode {
...
func (b *Bouncer) recordProcessed(ipType string) {
	if client := b.publishedLAPI(); client != nil {
...
func (b *Bouncer) handleBanServeHTTP(rw http.ResponseWriter, req clientRequest, reason, origin string) {
	...
	rw.WriteHeader(b.bouncerRemediationStatusCode)
...
func (b *Bouncer) applyAppsecServeHTTP(rw http.ResponseWriter, req clientRequest) bool {
	...
	decision, err := client.Query(req.remoteIP, req.Request, pol)
...
func (b *Bouncer) handleAppsecResponseServeHTTP(rw http.ResponseWriter, req clientRequest, decision *appsec.Response) {
	...
	if status < 100 || status > 999 {
		status = b.bouncerRemediationStatusCode
	}
