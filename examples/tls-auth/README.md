...
http:
  middlewares:
    crowdsec:
      plugin:
        bouncer:
          lapiTlsCertificateAuthorityFile: /etc/traefik/certs/crowdsecCA.pem
```
...
services:
  whoami-foo:
    ...
    labels:
      - |
        traefik.http.middlewares.crowdsec-foo.plugin.bouncer.lapiTlsCertificateAuthority=
        -----BEGIN CERTIFICATE-----
        MIIEBzCCAu+gAwIBAgICEAAwDQYJKoZIhvcNAQELBQAwgZQxCzAJBgNVBAYTAlVT
        MRAwDgYDVQQHDAdTZWF0dGxlMRMwEQYDVQQIDApXYXNoaW5ndG9uMSIwIAYDVQQK
        ...
...
To separate TLS config for LAPI and Appsec, you can use all the TLS LAPI variable beginning with `lapiTls...` into `appsecTls...`.
