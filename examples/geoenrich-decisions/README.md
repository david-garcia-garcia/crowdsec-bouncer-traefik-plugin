...
Nested maps (`bouncerDecisionScopeHeaders`, geoblock `databaseSources`) live in [`dynamic.yml`](dynamic.yml). Docker labels do not decode those maps reliably.
...
The compose file loads **this tree** as a local Traefik plugin so `bouncerDecisionScopeHeaders` works before it is in a catalog release. Geoblock is also local (`useunsafe` is required). Switch both to catalog plugins after a bouncer release that includes this key; keep geoblock `settings.useunsafe=true`.
...
- Bouncer `bouncerDecisionScopeHeaders.Country: X-IPCountry`.
...
See the plugin README `bouncerDecisionScopeHeaders` section for Country/AS normalize rules (`XX`/`T1` do not match).
